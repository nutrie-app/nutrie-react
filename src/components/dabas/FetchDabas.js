...
export const getGtin = gtin => {
    return (
        fetch(`${url}/article/gtin/${gtin}/json?apikey=${apikey}`)
        .then((response) => response.json())
        .then((responseJson) => {
        const {Allergener, Artikelbenamning, Bilder, Naringsinfo, Nettoinnehall, Tillverkningslander, Varumarke, Ingrediensforteckning} = responseJson
        const info = {
            Allergener,
            Artikelbenamning,
            Bilder,
...
export const getGtinHierarchy = gtin => {
    return (
        fetch(`${url}/articlehierarchy/gtin/${gtin}/json?apikey=${apikey}`)
        .then((response) => response.json())
        .then((responseJson) => {
...
export const getBasGtin = gtin => {
    return ( 
        getGtinHierarchy(gtin).then(response => {
            const {bas} = response
